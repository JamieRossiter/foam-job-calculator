                ...
                <div className="foam-card-content-container">
                    <div className="foam-left-container">
                        <div className="foam-density-container foam-card-sub-container">
                            ...
                            <Dropdown tabIndex={1} value={props.data.density} onChange={(e: any, data: DropdownProps) => { handleChange("density", data.value) }} fluid name="densities-select" id="densities-select" placeholder="Select density..." options={densityList} selection />
                        </div>
                        <div className="foam-length-container foam-card-sub-container">
                            <label htmlFor="dimensions-length">Length</label>
                            <Input tabIndex={2} value={props.data.length < 1 ? "" : props.data.length} onChange={(e: any, data: InputOnChangeData) => { handleChange("length", data.value) }} className="length-input" type="number" id="dimensions-length" labelPosition="right" placeholder="e.g. 500" label={<Dropdown onChange={(e: any, data: DropdownProps) => { handleChange("measurementSystem", data.value) }} options={measurementSystemList} defaultValue="mm" value={props.currentMeasurementSystem} />} />
                        </div> 
                    </div>
                    <div className="foam-right-container">
                        <div className="foam-thickness-container foam-card-sub-container">
                            ...
                            <Dropdown tabIndex={1} value={props.data.thickness} onChange={(e: any, data: DropdownProps) => { handleChange("thickness", data.value) }} fluid name="thicknesses-select" id="thicknesses-select" placeholder="Select thickness..." options={thicknessList} selection />
                        </div>
                        <div className="foam-width-container foam-card-sub-container">
                            <label htmlFor="dimensions-width">Width</label>
                            <Input tabIndex={2} value={props.data.width < 1 ? "" : props.data.width} onChange={(e: any, data: InputOnChangeData) => { handleChange("width", data.value) }} className="width-input" type="number" id="dimensions-width" labelPosition="right" placeholder="e.g. 300" label={<Dropdown onChange={(e: any, data: DropdownProps) => { handleChange("measurementSystem", data.value) }} options={measurementSystemList} defaultValue="mm" value={props.currentMeasurementSystem} />} />
                        </div>
                    </div>
                </div>
                <div className="foam-amount-container foam-card-sub-container">
                    <label htmlFor="foam-amount">Quantity</label>
                    <Input tabIndex={2} value={props.data.quantity} onChange={(e: any, data: InputOnChangeData) => { handleChange("quantity", data.value) }} type="number" id="foam-amount" defaultValue={0} label="cushions" labelPosition="right" />
                </div>
            </div>
        </>
    )
